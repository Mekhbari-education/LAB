...
export default function Chemicals({ isNested = false }: { isNested?: boolean }) {
  ...
  return (
    <div className={cn("space-y-10 max-w-7xl mx-auto pb-20", !isNested && "px-4")} dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
      {/* Header */}
      {!isNested && (
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-4">
          <div className="space-y-1">
            <h1 className="text-4xl font-black text-primary tracking-tighter">{t('chemicals.title', 'المخزن الكيميائي')}</h1>
            <p className="text-secondary/80 text-base font-medium">{t('chemicals.subtitle', 'إدارة وتتبع المحاليل والكواشف الكيميائية')}</p>
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            >
              <Download size={20} />
              {t('common.export_xls', 'تصدير الجرد')}
            </button>
            <button 
            ...
            >
              <Plus size={20} />
              {t('chemicals.btn_add', 'إضافة مادة')}
            </button>
          </div>
        </header>
      )}
      ...
      {!isNested && (
        <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-surface-container-low p-7 rounded-[32px] border border-outline/5 hover:border-outline/20 transition-all group">
            <p className="text-xs text-secondary/60 font-black uppercase tracking-widest mb-3">{t('chemicals.stat_total_chemicals', 'إجمالي المواد')}</p>
          ...
          <div className="bg-error-container/40 p-7 rounded-[32px] border border-error/10 hover:border-error/20 transition-all group">
            <p className="text-xs text-on-error-container/60 font-black uppercase tracking-widest mb-3">{t('chemicals.col_hazard', 'مواد خطرة')}</p>
          ...
          <div className="bg-tertiary-fixed/40 p-7 rounded-[32px] border border-tertiary/10 hover:border-tertiary/20 transition-all group">
            <p className="text-xs text-on-tertiary-fixed/60 font-black uppercase tracking-widest mb-3">{t('chemicals.stat_expired', 'تنتهي قريباً')}</p>
          ...
          <div className="bg-primary p-7 rounded-[32px] text-on-primary shadow-xl shadow-primary/20 hover:shadow-2xl transition-all group relative overflow-hidden">
            ...
            <div className="relative z-10">
              <p className="text-white/60 text-xs font-black uppercase tracking-widest mb-3">{t('equipment.field_quantity', 'سعة التخزين')}</p>
      ...
      {lowStockCount > 0 && (
        ...
        >
          <div className="flex items-center gap-4 text-error">
            ...
            <span className="font-black text-base">{t('chemicals.low_stock_badge', 'تنبيه: يوجد مواد منخفضة المخزون!')} ({lowStockCount})</span>
          </div>
          <button 
          ...
          >
            {filterLowStock ? t('common.all', 'عرض الكل') : t('chemicals.filter_low_stock', 'عرض المواد المنخفضة')}
          </button>
        </motion.div>
      )}
            ...
            <div className="p-8 flex flex-col md:flex-row justify-between items-center gap-6 bg-surface-container-low/30 border-b border-outline/5">
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  title={filterLowStock ? t('common.all', "عرض الكل") : t('chemicals.filter_low_stock', "تصفية المواد المنخفضة")}
                >
                  <Filter size={22} />
                </button>
              </div>
                  ...
                  <tr className="bg-surface-container-low/50 text-secondary/60 text-[11px] font-black uppercase tracking-widest">
                    ...
                    >
                      <div className="flex items-center">
                        {getSortIcon('nameEn')}
                        {t('chemicals.col_name_en', 'المادة (EN/AR)')}
                      </div>
                    </th>
                    <th 
                    ...
                    >
                      <div className="flex items-center">
                        {getSortIcon('formula')}
                        {t('chemicals.col_formula', 'الصيغة')}
                      </div>
                    </th>
                    <th 
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        {t('chemicals.col_quantity', 'الكمية')}
                      </div>
                    </th>
                    <th 
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        {t('chemicals.col_hazard', 'الخطورة')}
                      </div>
                    </th>
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        {t('chemicals.col_expiry', 'الصلاحية')}
                      </div>
                    </th>
                    <th className="px-3 py-5 text-right hidden 2xl:table-cell">{t('common.notes', 'ملاحظات')}</th>
                    <th className="px-3 py-5 text-center w-24">{t('common.actions', 'إجراءات')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-outline/5 relative w-full">
                  {loading ? (
                    <tr>
                      <td colSpan={12} className="px-8 py-20 text-center text-outline/60 font-bold">{t('common.loading', 'جاري التحميل...')}</td>
                    </tr>
                  ) : sortedChemicals.length === 0 ? (
                    <tr>
                      <td colSpan={12} className="px-8 py-20 text-center text-outline/60 font-bold">{t('common.empty', 'لا توجد مواد مطابقة للبحث')}</td>
                    </tr>
                  ) : (
                    <>
        ...
        <div className="lg:col-span-4 space-y-8">
          {selectedChemical ? (
              ...
              <div className="relative z-10 space-y-8">
                <div className="flex items-start justify-between">
                  <span className={cn(
                    ...
                    {selectedChemical.hazardClass === 'danger' ? t('chemicals.hazard_danger', 'مادة خطرة') : t('chemicals.hazard_safe', 'مادة آمنة')}
                  </span>
                ...
                <div className="space-y-5 pt-8 border-t border-outline/5">
                  <div className="flex justify-between items-center">
                    <span className="text-base font-bold text-secondary/60 uppercase tracking-widest">{t('chemicals.field_cas', 'رقم CAS')}</span>
                    <span className="font-black text-primary text-lg">{selectedChemical.casNumber || t('common.none', 'غير متوفر')}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-base font-bold text-secondary/60 uppercase tracking-widest">{t('chemicals.field_storage_temp', 'درجة التخزين')}</span>
                    <span className="font-black text-primary text-lg">{selectedChemical.storageTemp || t('common.none', 'غير متوفر')}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-base font-bold text-secondary/60 uppercase tracking-widest">{t('common.status', 'الحالة')}</span>
                    <span className="font-black text-primary text-lg">{selectedChemical.state === 'solid' ? t('chemicals.state_solid', 'صلب') : selectedChemical.state === 'liquid' ? t('chemicals.state_liquid', 'سائل') : t('chemicals.state_gas', 'غاز')}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-base font-bold text-secondary/60 uppercase tracking-widest">{t('chemicals.col_location', 'الرف')}</span>
                    <span className="font-black text-primary text-lg">{selectedChemical.shelf}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-base font-bold text-secondary/60 uppercase tracking-widest">{t('chemicals.col_expiry', 'الصلاحية')}</span>
                    <span className={cn(
                      "font-black text-lg",
                  ...
                  <div className="flex justify-between items-start">
                    <span className="text-base font-bold text-secondary/60 uppercase tracking-widest">{t('common.notes', 'ملاحظات')}</span>
                    <span className="font-black text-primary text-sm text-left flex-1 mr-4 leading-relaxed break-words">{selectedChemical.notes || t('common.none', 'لا توجد')}</span>
                  </div>

                  {selectedChemical.ghs && selectedChemical.ghs.length > 0 && (
                    <div className="pt-6 border-t border-outline/5">
                      <span className="text-[11px] font-black text-secondary/40 uppercase tracking-[0.2em] block mb-4">{t('chemicals.ghs_symbols', 'رموز السلامة GHS')}</span>
                  ...
                  <div className="space-y-3 pt-2">
                    <div className="flex justify-between items-end">
                      <span className="text-sm font-black text-primary uppercase tracking-widest">{t('chemicals.col_quantity', 'مستوى المخزون')}</span>
                ...
                <div className="flex gap-3 pt-4">
                  <button 
                    ...
                    title={t('common.print', 'طباعة تعريفية')}
                  >
                    <Printer size={22} />
                  </button>
                  <button 
                    ...
                    className="p-3 bg-surface-container-low hover:bg-surface-container-high border border-outline/10 rounded-full text-primary transition-all active:scale-90"
                    title={t('chemicals.btn_print_stock_cards', 'طباعة بطاقة المخزون')}
                  >
                    <FileText size={22} />
                  </button>
                  <button 
                    ...
                    title={t('equipment.btn_smart_update', 'تحديث ذكي للمعلومات')}
                  >
                    {isGenerating ? (
                  ...
                  <button className="p-3 bg-surface-container-low hover:bg-surface-container-high border border-outline/10 rounded-full text-primary transition-all active:scale-90" title={t('common.scan_qr', 'توليد رمز QR')}>
                    <QrCode size={22} />
                  </button>
                </div>
              </div>
            ...
            <div className="bg-surface-container-lowest rounded-[32px] p-12 text-center text-outline/60 font-bold border border-outline/10 border-dashed">
              {t('chemicals.select_prompt', 'اختر مادة من القائمة لعرض تفاصيلها المخبرية')}
            </div>
          )}
          ...
          <div className="bg-primary-container/30 backdrop-blur-sm p-8 rounded-[32px] text-on-primary-container border border-primary/10 relative overflow-hidden group shadow-sm">
            <div className="relative z-10">
              <h4 className="font-black text-lg mb-3 flex items-center gap-2 text-primary">
                <ShieldAlert size={20} />
                {t('chemicals.safety_instructions', 'تعليمات السلامة')}
              </h4>
