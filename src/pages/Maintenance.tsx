...
export default function Maintenance() {
  ...
  const { data: equipment } = 
    useSqlCollection<Equipment>('equipment', '/api/db/equipment');
  ...
  const handleAddLog = async (e: React.FormEvent) => {
    ...
    try {
      await createMaintenanceLog({
        ...
        cost: newLog.cost ? String(newLog.cost) : undefined,
      });
      ...
      alert('فشل إضافة سجل الصيانة إلى قاعدة البيانات');
    }
  };
