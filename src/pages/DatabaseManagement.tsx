  ...
  const fetchStats = async () => {
    ...
    try {
      const newStats = await Promise.all(stats.map(async (stat) => {
        const snapshot = await getDocs(getUserCollection(schoolId, stat.name));
  ...
  const handleImportJSON = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const stat of stats) {
        if (importedData[stat.name] && Array.isArray(importedData[stat.name])) {
          for (const item of importedData[stat.name]) {
            const { id, ...data } = item;
            if (id) {
              const docRef = doc(getUserCollection(schoolId, stat.name), id);
      ...
      alert('حدث خطأ أثناء قراءة أو استيراد الملف. تأكد من صحة التنسيق.');
    } finally {
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-surface-container-lowest rounded-[32px] p-8 border border-outline/10 shadow-sm space-y-6">
            ...
            <button 
              ...
              className={cn(
                "w-full flex items-center justify-between p-5 bg-surface-container-low hover:bg-primary/5 rounded-2xl border border-outline/5 transition-all group",
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-right">
                  <p className="font-bold text-secondary">{isImporting ? 'جاري الاستيراد...' : 'استيراد بيانات'}</p>
