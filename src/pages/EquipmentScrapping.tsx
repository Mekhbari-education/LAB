...
export default function EquipmentScrapping() {
  ...
  const { data: sqlEquipment } = useSqlCollection<Equipment>('equipment', '/api/db/equipment');
  const { data: sqlTeachers } = useSqlCollection<any>('teachers', '/api/db/teachers');

  useEffect(() => {
    if (sqlEquipment.length > 0) {
      setEquipmentList(sqlEquipment);
    }
    if (sqlTeachers.length > 0) {
      setTeachersList(sqlTeachers);
    }
  }, [sqlEquipment, sqlTeachers]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const savedRecord = await createEquipmentScrapping({
        pvNumber: proposalData.num || `PV-${Date.now()}`,
        date: proposalData.date || new Date().toISOString().split('T')[0],
        location: schoolName || 'المخبر',
        committeeMembers: pvData.members || [],
      ...
      await logActivity(schoolId, LogAction.CREATE, LogModule.EQUIPMENT, `سجل إسقاط بملفات مرفقة رقم: ${proposalData.num}`, savedRecord.id);
      setNotification({ message: 'تم حفظ السجل والملفات بنجاح في قاعدة البيانات السحابية PostgreSQL!', type: 'success' });
    } catch (e: any) {
