...
import { 
  ...
  createLabSession, 
  updateLabSession 
} from '../lib/api/studentClasses';
import { createGlassware } from '../lib/api/glassware';
import { Users, Grid, Plus, Trash2, Calendar, User, MessageSquare, CheckCircle2, ShieldAlert, Play, Square, Beaker, Save, X, WifiOff } from 'lucide-react';
...
export default function StudentGroups() {
  ...
  const { data: classesData, loading: classesLoading, invalidate: invalidateClasses, isOffline } = 
    useSqlCollection<StudentClass>('student_classes', '/api/db/student-classes');
  const { data: sessionsData, loading: sessionsLoading, invalidate: invalidateSessions } = 
    useSqlCollection<LabSession>('lab_sessions', '/api/db/lab-sessions');
  const { data: teachersData } = 
    useSqlCollection<any>('teachers', '/api/db/teachers');

  const classes = [...classesData].sort((a, b) => (a.level || '').localeCompare(b.level || ''));
  ...
  useEffect(() => {
    const currentActive = sessions.find(s => s.status === 'active');
    if (currentActive) {
      setActiveSession(currentActive);
    } else if (activeSession && !sessions.find(s => s.id === activeSession.id && s.status === 'active')) {
  ...
  const handleStartSession = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const created = await createLabSession({
        ...
        benches: Number(newSession.benchCount) || 12,
        status: 'active'
      });
  ...
  const handleEndSession = async () => {
    ...
    if (confirm('إنهاء وحفظ جلسة العمل التطبيقي؟')) {
      try {
        await updateLabSession(activeSession.id, {
          status: 'completed'
        });
        setActiveSession(null);
      } catch (error) {
  ...
  const handleReportIncident = async (e: React.FormEvent) => {
    ...
    try {
      await createGlassware({
        nameAr: `كسر في طاولة ${incidentModal.benchNum}`,
        designationFr: `Casse paillasse ${incidentModal.benchNum}`,
        type: 'مكسور',
        unit: 'قطعة',
        quantity: 1,
        status: 'مكسورة',
        location: activeSession.roomName,
        notes: `القسم: ${activeSession.className} - ${activeSession.groupName}. الأستاذ: ${activeSession.teacherName}. البيان: ${incidentNote}`
      });
      setIncidentModal({ isOpen: false, benchNum: null });
      setIncidentNote('');
      alert('تم تسجيل الحادثة بنجاح وحفظها في قاعدة البيانات السحابية PostgreSQL.');
    } catch (error) {
                ...
                <tbody className="divide-y divide-outline-variant/30">
                  {sessions.filter(s => s.status === 'completed').map(s => (
                    <tr key={s.id} className="hover:bg-secondary-container/20 transition-colors">
                      <td className="p-4 text-sm font-semibold">{s.date ? (typeof (s.date as any)?.toDate === 'function' ? (s.date as any).toDate().toLocaleDateString('ar-DZ') : new Date(s.date).toLocaleDateString('ar-DZ')) : 'N/A'}</td>
