...
export default function GlasswareBreakage({ isNested = false }: { isNested?: boolean }) {
  ...
  const handleSeedData = async () => {
    ...
    try {
      await createGlasswareBulk(INITIAL_GLASSWARE);
      alert('تمت إضافة 25 صنف بنجاح إلى قاعدة بيانات PostgreSQL!');
    } catch (error) {
  ...
  const handleUpdateItem = async (id: string, updates: Partial<GlasswareItem>) => {
    try {
      await updateGlassware(id, updates);
    } catch (error) {
