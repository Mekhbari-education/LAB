...
import { doc, getDoc, setDoc, writeBatch, serverTimestamp, getDocFromServer, deleteDoc } from 'firebase/firestore';
...
export default function SettingsPage() {
  ...
  const fixPermissions = async () => {
    ...
    setFixResults([]);
    const tests = [
      { name: 'الاتصال العام (Root)', path: '_connection_test_/ping', data: { _permission_test: true, lastTested: serverTimestamp(), clientTime: new Date().toISOString() } },
      { name: 'ملف المستخدم (User Profile)', path: `users/${auth.currentUser.uid}`, data: { _permission_test: true, lastTested: serverTimestamp(), clientTime: new Date().toISOString() } },
      { name: 'مجموعة المواد (Chemicals)', path: `schools/${schoolId}/chemicals/permission_test`, data: { name: 'Test Chemical', quantity: 1, unit: 'g' } },
      { name: 'مجموعة الإعدادات (Settings)', path: `schools/${schoolId}/settings/permission_test`, data: { school: 'Test School', directorate: 'Test Directorate' } },
    ];

    const results = [];
    for (const test of tests) {
      try {
        await setDoc(doc(db, test.path), test.data, { merge: true });
        results.push({ name: test.name, success: true });
        // Clean up the dummy data right after
        if (test.path.endsWith('/permission_test')) {
            try { await deleteDoc(doc(db, test.path)); } catch (e) {}
        }
      } catch (err: any) {
