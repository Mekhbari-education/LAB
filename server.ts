...
/**
 * مخزن Redis للتحكم في عدد الطلبات لسيرفرات Scale-out.
 */
...
class RedisStore implements RateLimitStore {
  ...
  async increment(key: string, windowMs: number) {
    ...
    // Set TTL if it's a new key (-1 means no TTL, -2 means doesn't exist but incr created it without ttl)
    if (ttl === -1 || count === 1) {
      await this.redis.expire(redisKey, windowSeconds);
      ttl = windowMs;
    } else if (ttl < 0) {
       ttl = windowMs; // Fallback
    }
...
const activeRateLimitStore = process.env.REDIS_URL 
  ? new RedisStore(new Redis(process.env.REDIS_URL)) 
  : new MemoryStore();

function createRateLimiter(windowMs: number, maxRequests: number, store: RateLimitStore = activeRateLimitStore) {
...
async function startServer() {
  ...
  app.post('/api/setup-admin', requireLocalhost, adminLimiter, async (req, res) => {
    try {
      const { email, serviceAccountJson, idToken } = req.body;
      if (!email || !serviceAccountJson || !idToken) {
        return res.status(400).json({ error: 'Missing email, serviceAccountJson, or idToken' });
      }
  ...
  app.post('/api/migrate-schools', requireLocalhost, adminLimiter, async (req, res) => {
    try {
      const { serviceAccountJson, idToken } = req.body;
      if (!serviceAccountJson || !idToken) {
        return res.status(400).json({ error: 'Missing serviceAccountJson or idToken' });
      }
  ...
  // Mount dbRoutes (Drizzle / PostgreSQL)
  const dbRoutesModule = await import('./src/routes/dbRoutes.ts');
  app.use('/api/db', dbRoutesModule.default);
